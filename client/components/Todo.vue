  ...
  <div>
    ...
    <section v-else>
      ...
      <section v-if="todos.length == 0">
        <center>No todos created. Create one by clicking on the plus icon in the bottom right</center>
      </section>
      <section v-else>
          ...
          <v-card width="500">
            <section v-if="edititem && index==edititem.index">
              ...
              <v-card-actions>
                <v-btn
                  color="yellow"
                  @click="editTodo(edititem.id, edititem.index, edititem.title, edititem.description)"
                >
                  EDIT TODO
                </v-btn>
                <v-btn
            ...
            <section v-else>
              ...
              <v-card-actions>
                <v-checkbox
                  ...
                  :label="todo.isComplete ? 'Completed':'Incomplete'"
                  color="pink"
                  @click="changeStatus(todo.id, index, todo.isComplete)"
                />
                <v-spacer />
                <v-tooltip top>
                  <template v-slot:activator="{ on }">
                    <v-btn
                      ...
                      color="yellow"
                      v-on="on"
                      @click="showEditBox(todo.id, index)"
                    >
                      <v-icon>edit</v-icon>
                    </v-btn>
                  </template>
